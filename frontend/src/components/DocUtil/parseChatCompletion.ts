...
export const KB_WRITING_CONSTRAINT_PROMPT =
  ...
  '3. 检索里已经出现的数字必须写出来。百分数和名称可以分成相邻短句，不必和大纲要点连成同一句。大纲用词和检索用词不完全一致时，以检索用词为准。禁止整页拒绝，禁止写「知识库未提供」「知识库中未提供」「无法据此」。只有该要点的数字和专名在检索结果中都完全没有出现时，这一条的描述才写「知识库未提供依据」。专名已经出现时，标题用要点里的专名，不要写成「打法适用情境」。\n' +
  '4. 具体数字、专有名词须与检索内容一致，可在句末用 [1][2] 标注引用。\n' +
  '5. 必须按【输出格式】逐行编号输出（1. 标题 - 描述）。数据卡行以原数字开头；禁止整段叙述、禁止只写一段话。\n';
