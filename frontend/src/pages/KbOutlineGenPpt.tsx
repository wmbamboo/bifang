...
import {Button, Empty, Flex, message, Progress, Spin, Tag} from 'antd';
...
/*const addPptOutlineRecToStorage=(markdown:string,outlineRecs:OutlineRec[])=>{
  ...
  OutlineRec.save(outlineTypePPT,outlineRec_init)
...
/*const get_kbName_from_outlineRecs=(outlineRecs:OutlineRec[],idx:number)=>{
  init_kbName=outlineRecs[idx].outline_source_kbName;
...
/*const outlineRecs:OutlineRec[]= OutlineRec.listRecs(outlineTypePPT);
let init_title="";
let init_content: string;
let init_chapters:Chapter[]= [];
let init_kbName:string;
...
init_kbName=get_kbName_from_outlineRecs(outlineRecs,0)*/
...
const KbOutlineGenPpt: React.FC = () => {
  ...
  useEffect(() => {
    const recs = OutlineRec.listRecs(outlineTypePPT, kbName);
    setOlRecs(recs);
  ...
  const onOutlineRecSelect=useCallback( (id:string)=>{
    // 按 ID 从全量列表取，避免同名大纲或知识库筛选导致取错份。
    const allRecs = OutlineRec.listRecs(outlineTypePPT);
    const [newTitle, , outlineKb, newChapters]=Ppt.get_chapters_from_outlineRecs(allRecs, outlineTypePPT, id, formatPrompt)
    if(newTitle && newChapters && newChapters.length > 0) {
      setChapters(newChapters);
      setTitle(newTitle);
      setDownloadable(false);
      // 大纲所属库与当前筛选不一致时，切到该大纲的库，便于列表与内容一致
      if (outlineKb && outlineKb !== kbName) {
        setKbName(outlineKb);
      }
      message.info(`已载入大纲：${newTitle}（${newChapters.length} 章）`);
      return true
    }
    message.warning(`出错了，ID为${id}的大纲中，主题或章节无内容`);
    return false;
  },[kbName, formatPrompt]);
  ...
  function onOutlineRecDelete(id:string) {
    ...
    if(or) {
      OutlineRec.deleteRecById(or.outlineId, outlineTypePPT);
      let newRecs = OutlineRec.listRecs(outlineTypePPT, kbName);
      setOlRecs(newRecs)
  ...
  const beforeOutlineSelect =()=>{
    ...
    setOlRecs(recs);
    if(recs.length===0){
      message.warning(`知识库「${kbLabel(kbName)}」下尚无 PPT 大纲，请先在【大纲构思】中生成并保存。`);
      return false;
    }
    setDrawerOpen(true);
  }
  ...
  const genDoc = useCallback(async (genKey:string="") => {
    message.info(`genPPT:ID:${title},${kbName},${chapters.length}`);
    setLoading(true);
    // 需要使用kbName
        ...
        for (let slide of chapter.slides) {
          ...
          if (genKey.length === 0) {
            if (slide.prompt.length > 0) {
              promiseArr.push(axios.post(url, buildMsg(slide.prompt + KB_WRITING_CONSTRAINT_PROMPT + buildPptItemFormatPrompt(countOutlineTips(slide.subTitle), slide.layout, (() => { const p = splitMetricListTips(slide.subTitle || ''); return slide.layout === 'metric_list' ? { metric: p.metrics.length, list: p.lists.length } : undefined; })())), {headers: headers}));
  ...
  function cb4ImportOutline(title:string,rawContent:string) {
    ...
    Ppt.addDocOutlineRecToStorage(outlineTypePPT, msgContent, kbName);
    let newRecs=OutlineRec.listRecs(outlineTypePPT, kbName)
    setOlRecs(newRecs)
...
export default KbOutlineGenPpt;
