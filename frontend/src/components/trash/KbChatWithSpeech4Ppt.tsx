...
const KbChatWithSpeech4Ppt=(props:ChatWithSpeechProps)=>{
  return (
    <ProChatProvider>
      <Chat kb_name={props.kb_name} openai={props.openai}
            cb4setOutlineRec={props.cb4setOutlineRec} cb4setTempOutlineRecs={props.cb4setTempOutlineRecs}/>
      <ChatControlBar key={"chatControlBar"} buttonMessages={props.buttonMessages} />
    </ProChatProvider>
  )
}
    ...
    <ProChat
      ...
      chatItemRenderConfig={{
        actionsRender: (props, dom, actionsProps) => {
          ...
          if (props?.editing || msg.length<=50 || !msg.includes("##")) {
            return null;
          }
          return (
            <>
              <Button size={"small"} style={{marginTop:"0.5em"}}
                      type="dashed"
                      onClick={() => {
              ...
              <Button size={"small"} style={{marginTop:"0.5em"}}
                      ...
                      onClick={(e) => {
                        ...
                        if(msg.substring(0,11)==="```markdown") {
                          // openNotificationWithIcon("info","大纲已标识为markdown。请注意```标识后不能有任何字符，有的话请删除")
                          ...
                          // openNotificationWithIcon("info","大纲未标识为markdown。")
                          content = msg;
                        }
                        console.log(content);
                        //TODO HEZL
                        ...
                        let chapters=Ppt.getChaptersFromContent(recContent)
                        if(!chapters||chapters.length===0){
                          message.error("无法保存，PPT章节应该以【## 】开头单独一行，幻灯片应该以【### 】开头单独一行，副标题为【* 】开头单独一行，请手工检查并修改。")
                          return;
                        }
                        let newPptOutlineRec=new OutlineRec(recTitle,recContent);
                        console.log("the new OutlineRec:", JSON.stringify(newPptOutlineRec));
                        cb4setOutlineRec(newPptOutlineRec);
                        OutlineRec.save(outlineTypeAiPPT,newPptOutlineRec)
...
export default KbChatWithSpeech4Ppt;
